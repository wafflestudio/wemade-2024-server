    ...
    def __call__(self, value):
        ...
        for key, val in value.items():
            ...
            if col.is_public != self.expected_public:
                errors[key] = (
                    f"이 컬럼은 {'공개' if col.is_public else '비공개'} 정보입니다. 적절한 필드에 입력하세요."
                )
            ...
            if col.column_type == "txt":
                if not isinstance(val, str) and not (
                    ...
                    errors[key] = (
                        "텍스트 형식은 문자열 또는 'value' 키를 포함하는 객체여야 합니다."
                    )
                ...
                if not isinstance(val, str) and not (
                    ...
                    errors[key] = (
                        f"{col.get_column_type_display()} 형식은 문자열 또는 'value' 키를 포함하는 객체여야 합니다."
                    )
            ...
            if col.is_supporting_material_required:
                if isinstance(val, dict):
                    if (
                        "supporting_material" not in val
                        or not val["supporting_material"]
                    ):
                        errors[key] = (
                            "증빙자료가 필요한 컬럼입니다. 'supporting_material' 값을 제공하세요."
                        )
                else:
                    errors[key] = (
                        "증빙자료가 필요한 컬럼은 객체 형태로 입력되어야 합니다."
                    )
