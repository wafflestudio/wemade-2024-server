...
urlpatterns = [
    ...
    path('<int:p_id>/history/list/', RoleHistoryListAPIView.as_view(), name='personal-history-list'),
    # 직무 히스토리 내 직무 설명(job description) 수정하기 (본인/Master/HR Team)
    path('<int:p_id>/history/update/<int:role_id>', RoleHistoryUpdateAPIView.as_view(), name='personal-history-update'),
